...
39) Изменяем внутренний IP-адрес мастерноды в run_generate_script.py (находится на ВМ c AirFlow)

		sudo nano /home/airflow/dags/run_generate_script.py

	remote_host в SSHHook должен соответствовать внутреннему IP адресу мастерноды

40) Изменяем IP-адрес сервера MLFlow на дествительный в файле fit.py, строка mlflow.set_tracking_uri(...) (находится на DataProc)

		sudo nano /MLOps/airflow_dataproc_mlflow/for_dataproc/fit.py
